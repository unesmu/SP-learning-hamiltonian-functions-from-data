...
def get_trajectory_pend(device, time_steps, Ts, y0, noise_std, C, m, g, l, u_func, g_func):
    ...
    q_p = odeint(func=lambda t, coords: dynamics_fn_pend(t, coords, C, m, g, l,
                 u_func, g_func), y0=y0, t=t_eval, method='rk4', options=dict(step_size=Ts))
...
def multiple_trajectories(time_steps, num_trajectories, device, Ts, y0,
    ...
    energy, derivatives = get_energy_pendulum(
        t_eval, u_func, g_func, q, p, C, m, g, l)
    ...
    for _ in range(num_trajectories-1):
        ...
        derivatives = torch.vstack(
            (derivatives, derivatives_n.unsqueeze(dim=0)))
