...
def plot_traj_pend(t_eval, q, p, energy=torch.tensor(False), input=torch.tensor(False),
                   coord_type='hamiltonian', title='Trajectory of the generalized coordinates', save_plot=False, file_path=None):
    '''
...
def train_test_loss_plot(loss_train, loss_test, epochs, test_epochs, file_path=None,
    ...
    if horizons:
        ...
        ax.annotate(
            ...
            xy=(epochs[horizon_steps[-1]],
                loss_train[horizon_steps[-1]]), xycoords='data',
...
def plot_results(model_training, n, train_horizon,
                 file_path, show_pred=True, train = True,
    ...
    energy_nom, _ = get_energy_pendulum(
        t_eval, model_training.u_func, model_training.g_func, x[n, :, 0], x[n, :, 1], model_training.C, model_training.m, model_training.g, model_training.l)
    energy_pred, _ = get_energy_pendulum(
        t_eval, model_training.u_func, model_training.g_func, test_x_hat[:, 0, 0], test_x_hat[:, 0, 1], model_training.C, model_training.m, model_training.g, model_training.l)
    ...
    for q, p, energy, label, linewidth, color in [(x[n, :, 0], x[n, :, 1], energy_nom, 'nominal', 2, 'C0'), (test_x_hat[:, 0, 0], test_x_hat[:, 0, 1], energy_pred, 'predicted', 1, 'r')]:
        q = q.cpu().detach()
        p = p.cpu().detach()
        ...
        if only_pred == False and label == 'nominal':  
            ax1.plot(t_eval[:train_horizon], q[:train_horizon],
                     label='train', color='g')
            ax2.plot(t_eval[:train_horizon], p[:train_horizon],
                     label='train', color='g')
            ax3.plot(t_eval[:train_horizon],
                     energy[:train_horizon], label='train', color='g')
