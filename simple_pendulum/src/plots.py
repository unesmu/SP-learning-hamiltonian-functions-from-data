...
coord_type = 'hamiltonian', title = 'Trajectory of the generalized coordinates', save_plot = False, file_path = None):
    ...
    if torch.any(energy):
      if torch.any(input):
        fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 4,figsize=(15, 4),constrained_layout=True)#, sharex=True)# , sharey=True)
...
def plot_results(model, train_loader, train_horizon, n, u_func, g_func, C, m, g, l,
 file_path, show_pred = True,
  title = 'Trajectory of the generalized coordinates',
   ...
   only_pred = False):
    ...
    for q, p, energy, label, linewidth,color in [(x[n,:,0],x[n,:,1], energy_nom, 'nominal',2,'C0'),(test_x_hat[:, 0, 0], test_x_hat[:, 0, 1], energy_pred, 'predicted',1,'r')]:
        ...
        # if only_pred:
        #     if label == 'train':
        #         ax1.plot(t_eval, q, label=label, color='r')
        #         ax3.plot(t_eval, energy, label=label, color='r')
        #         ax2.plot(t_eval, p, label=label, color='r')
        #     else:
        ...
        if only_pred==False and label=='nominal' : # show_pred==True and
